...
import type { ChatModelOverride } from './chat-model-ref.types';
import { pushUniqueTrimmedSelectOption } from './select-options';
import type { ModelCatalogEntry, SessionsListResult } from './types';

/**
 * Local input shape — upstream uses `Pick<AppViewState, ...>` against the dash
 * UI's god-state. Helm composes the same fields independently.
 */
export type ChatModelSelectStateInput = {
  sessionKey: string;
